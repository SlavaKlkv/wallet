...
class TestWalletsErrors:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ('client', 'get', WALLET_DETAIL_URL, None,
             HTTPStatus.UNAUTHORIZED, False),
            ('auth_client_2', 'get', WALLET_DETAIL_URL, None,
             HTTPStatus.FORBIDDEN, False),
            ('auth_client', 'get', WALLET_DETAIL_URL, None,
             HTTPStatus.NOT_FOUND, True),
            # POST deposit (401, 403, 404)
            ('client', 'post', WALLET_OPERATION_URL,
                {'operation_type': 'deposit', 'amount': '100'},
             HTTPStatus.UNAUTHORIZED, False),
            ('auth_client_2', 'post', WALLET_OPERATION_URL,
                {'operation_type': 'deposit', 'amount': '100'},
             HTTPStatus.FORBIDDEN, False),
            ('auth_client', 'post', WALLET_OPERATION_URL,
                {'operation_type': 'deposit', 'amount': '100'},
             HTTPStatus.NOT_FOUND, True),
    ...
    def test_wallet_access_permissions(
            self, request, wallet, client_fixture,
            method, url, payload, expected_status, not_found
    ):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ({'operation_type': 'wrong', 'amount': '10'}, 'operation_type',
             'Некорректный тип операции.'),
            # Сумма меньше минимума
            ({'operation_type': 'deposit', 'amount': '0'}, 'amount',
             'Убедитесь, что это значение больше либо равно'),
            # Отрицательные значение
            ({'operation_type': 'withdraw', 'amount': '-1'}, 'amount',
             'Убедитесь, что это значение больше либо равно'),
        ]
    )
    def test_wallet_operations_bad_request(self, auth_client, wallet, payload,
        ...
        if isinstance(messages, list):
            ...
            assert (
                expected_message in str(
                    messages) or DRF_INVALID_CHOICE in str(messages)
            ), (
                f"Сообщение об ошибке должно содержать '{expected_message}' "
                f"или '{DRF_INVALID_CHOICE}'. Было: {messages}"
            )
